...
export const GET: APIRoute = ({ site }) => {
  ...
  const apps = getApps();
  const kits = apps.filter((app) => app.featured);
  const urls = ['', '/kits', '/directory', '/open-source', '/news', '/sell', ...apps.map((app) => `/${app.slug}`), ...kits.flatMap((app) => [`/buy/${app.slug}`, `/demos/${app.slug}`])];
